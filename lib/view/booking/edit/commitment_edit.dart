...
class CommitmentEdit extends StatefulWidget {
  const CommitmentEdit({Key? key}) : super(key: key);

  @override
  _CommitmentEditState createState() => _CommitmentEditState();
}

class _CommitmentEditState extends State<CommitmentEdit> {
  final commitController = Get.put(CommitmentController());
  List<String> commit_id = [];
  ...
  assignVal()
  {
    var data = commitController.commitment.value.result![0];
    commit_id = data.commitment!.split(',');
                      ...
                      case Status.COMPLETED:
                        return ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            if (commit_id.contains(commitData.id)) {
                              debugPrint(commitData.id);
                              init_check = true;
                              commitController.setCheckboxValue(index, true);
                            }
                            return Obx(() {
                              return Padding(
